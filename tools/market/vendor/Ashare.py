# Vendored from https://github.com/mpquant/Ashare (MIT) — A 股免费行情备选源
# -*- coding: utf-8 -*-
import datetime
import json
...
def get_price_day_tx(code, end_date="", count=10, frequency="1d"):
    ...
    url = (
        ...
        f"param={code},{unit},,{end_date},{count},qfq"
    )
    st = json.loads(requests.get(url, timeout=15).content)
    ms = "qfq" + unit
    stk = st["data"][code]
...
def get_price_sina(code, end_date="", count=10, frequency="60m"):
    ...
    dstr = json.loads(requests.get(url, timeout=15).content)
